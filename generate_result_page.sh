...
echo "___STATUS___<br/>" >> result.html
...
if [ -e "driven.csv" ]
then
    echo "<a href=\"driven.csv\" target=\"_new\">ANALYSIS RESULT: LIST OF INPUT NODES TARGETED BY DRUGS</a><br/>" >> result.html
fi
if [ -e "extra.csv" ]
then
    echo "<a href=\"extra.csv\" target=\"_new\">ANALYSIS RESULT: LIST OF INPUT NODES (NOT KNOWN TO BE TARGETED BY DRUGS)</a><br/>" >> result.html
fi
if [ -e "details.txt" ]
then
    echo "<a href=\"details.txt\" target=\"_new\">ANALYSIS RESULT: REPORT</a><br/>" >> result.html
fi
if [ -e "result.zip" ]
then
...
echo "<summary>PIPELINE TECHNICAL LOG (click to view)</summary><br/>" >> result.html
...
echo "<a href=\"session_control.php\" target=\"frame\">CLICK FOR PIPELINE EMERGENCY STOP</a> (WARNING! Emergency stop cannot be reverted. All current progress will be lost!)<br/>" >> result.html
echo "</details>" >> result.html
